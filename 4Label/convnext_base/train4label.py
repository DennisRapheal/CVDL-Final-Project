...
class CFG:
    ...
    backbone = "convnext_base" # convnext_base.fb_in22k_ft_in1k
    num_classes = 4
    drop_path_rate = 0.2
    lr = 1e-4; weight_decay = 1e-3; max_grad_norm = 1000
    scheduler = "cosine"; t_max = 20; min_lr = 5e-7
...
class NetClassifier(nn.Module):
    def __init__(self):
        ...
        self.pool=GeM(); self.head=nn.Sequential(nn.Linear(in_ch,512),nn.SiLU(),nn.Dropout(.3),nn.Linear(512,CFG.num_classes))
...
def train_epoch(loader, model, opt, sched, epoch):
    ...
    for step, (imgs, labels) in enumerate(loader):
        ...
        loss = criterion_hard(model(imgs), targets)
