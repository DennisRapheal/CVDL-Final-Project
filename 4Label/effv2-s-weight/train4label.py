...
class CFG:
    ...
    scheduler = "cosine"; t_max = 100; min_lr = 1e-7
    mixup_prob = 0.5; mixup_alpha = 0.4; cutmix_alpha = 1.0; label_smoothing = 0.1
...
class NetClassifier(nn.Module):
    def __init__(self):
        ...
        # 對每層都用 GeM pooling
        self.pooling = nn.ModuleList([GeM() for _ in range(self.num_layers)])

        # 對每層設一個加權參數（可訓練）
        self.layer_weights = nn.Parameter(torch.ones(self.num_layers) / self.num_layers)

        # 分類頭：將 concat 過的 pooled features 接 fc
        total_features = sum(self.in_channels_list)
    ...
    def forward(self, x):
        ...
        pooled = [self.pooling[i](f).squeeze(-1).squeeze(-1) for i, f in enumerate(feats)]  # List of [B,C]
        
        # 可以選擇 concat 或 weighted sum
        # 這裡我們 concat，並透過每層特徵對齊權重（用 softmax 控制）
        weights = torch.softmax(self.layer_weights, dim=0)  # shape: [num_layers]
        fused = torch.cat([w * f for w, f in zip(weights, pooled)], dim=1)  # [B, total_C]
