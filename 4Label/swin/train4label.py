...
class CFG:
    ...
    mixup_prob = 0.6; mixup_alpha = 0.4; cutmix_alpha = 1.0; label_smoothing = 0.1
...
class SwinV2Classifier(nn.Module):
    def __init__(self, num_classes=CFG.num_classes):
        ...
        in_ch = self.backbone.feature_info.channels()[-1]  # 1024 for SwinV2 base
        self.pool = nn.AdaptiveAvgPool2d(1)  # 使用 GAP 代替 GeM
...
def main():
    ...
    model=nn.DataParallel(SwinV2Classifier()).to(DEVICE)
